...
class ChangeUserInfoForm(forms.ModelForm): #Класс напрямую связан с полями модели пользователя
...
class RegisterUserForm(forms.ModelForm):  #Класс напрямую связан с полями модели пользователя
    email = forms.EmailField(required=True, label='Адрес электронной почты')
